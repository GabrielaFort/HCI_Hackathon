...
if run:
    ...
    # canonical oncotree names for dropdown (only these can be mapped)
    canonical_names = list(oncotree_map.keys())

    # Validate model's name: if blank, "unknown", or not in canonical list -> require override
    ...
    if onco_lower == "unknown" or is_onco_blank or (onco_pred and onco_pred not in canonical_names):
        ...
        # Show dropdown with canonical names only
        # default selection: first canonical name (or index 0)
        ...
        # override confirmation for onco name
        if "override_confirmed_name" not in st.session_state:
            st.session_state["override_confirmed_name"] = False

        if st.button("Override and continue with selected OncoTree name"):
        ...
        # use the chosen canonical name after confirmation
        final_onco_name = chosen_onco_name
    else:
        # model provided an accepted canonical name
